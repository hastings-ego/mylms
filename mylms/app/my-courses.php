...
<head>
    ...
    <title>My Courses | Fun Maths Mastery</title>
...
<body class="bg-slate-50 text-slate-900 font-sans antialiased h-screen overflow-hidden flex">
    ...
    <!-- Sidebar Navigation (identical to dashboard.php) -->
        ...
        <div class="flex flex-col h-full">
            ...
            <div class="flex-1 overflow-y-auto px-4 py-6 flex flex-col">
                <nav class="space-y-2 flex-1">
                    ...
                    <a href="my-courses.php" class="w-full flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-lg bg-brand-50 text-brand-700">
    ...
    <main class="flex-1 overflow-y-auto flex flex-col relative w-full">
        ...
        <div class="p-6 md:p-10 lg:p-12 max-w-6xl mx-auto w-full animate-fade-in">
            <h1 class="text-2xl md:text-3xl font-extrabold text-slate-900">My Courses & Resources</h1>
            <p class="text-slate-500 mt-2">All the digital products you've purchased.</p>
            ...
            <?php if (empty($purchased)): ?>
                <div class="mt-10 text-center py-12 bg-white rounded-xl border border-slate-200">
                    ...
                    <p class="text-slate-500 mb-4">You haven't purchased any resources yet.</p>
                    <a href="store.php" class="inline-block px-6 py-3 bg-brand-600 text-white font-semibold rounded-lg hover:bg-brand-700">Browse Store →</a>
                </div>
            <?php else: ?>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                    <?php foreach ($purchased as $item): ?>
                        <div class="border border-slate-200 rounded-xl bg-white hover:shadow-md transition-shadow overflow-hidden">
                            <div class="p-5">
                                <div class="flex justify-between items-start mb-3">
                                    <h3 class="text-lg font-bold text-slate-900"><?= h($item['title']) ?></h3>
                                    <span class="px-2 py-1 text-xs rounded-full <?= $item['file_type'] === 'pdf' ? 'bg-indigo-100 text-indigo-800' : 'bg-slate-100 text-slate-800' ?>">
                                        <?= strtoupper($item['file_type']) ?>
                                    </span>
                                </div>
                                <p class="text-sm text-slate-600 mb-4 line-clamp-2"><?= h(substr($item['description'], 0, 100)) ?>...</p>
                                <div class="flex justify-between items-center text-sm text-slate-500 mb-4">
                                    <span>Purchased: <?= date('d M Y', strtotime($item['purchase_date'])) ?></span>
                                    <span class="font-semibold text-slate-900">R <?= number_format($item['price'], 2) ?></span>
                                </div>
                                <a href="download.php?product_id=<?= $item['id'] ?>" class="block w-full text-center px-4 py-2 bg-brand-600 text-white font-semibold rounded-lg hover:bg-brand-700 transition-colors">
                                    Access Resource →
                                </a>
                            </div>
                        </div>
